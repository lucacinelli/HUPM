    ...
    for occ in (10, 15, 20):
        ...
        for k_sample in (10, 20, 50, 100):
            ...
            for random_transaction_index in sampled:
                ...
                if all_pred:
                    final_pred = next(map(f_norm, sum(map(lambda x: x[0][0] * x[0][1] * x[0][2], all_pred)) / sum(map(lambda x: x[1][0] * x[1][1], all_pred))))
                    pred_and_actual_values.append((random_transaction_index, final_pred, float(transaction.ICU.values[0])))
            
            n_trans_no_patterns = k_sample - len(pred_and_actual_values)
            precision = len(list(filter(lambda v: v[1] == v[2], pred_and_actual_values))) / float(len(pred_and_actual_values))
