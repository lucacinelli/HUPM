...
filename = None
menu_def = [['&File', ['&Open     Ctrl-O', '&Save       Ctrl-S', '&Properties', 'E&xit']], ['&Parameter',
                                                                                            ['Occurrences',
                                                                                             ['Occ 1', 'Occ 2', 'Occ 3',
                                                                                              'Occ 4', 'Occ 5', 'Occ 6',
                                                                                              'Occ 7', 'Occ 8', 'Occ 9',
        ...
        try:
            ...
            if "TRAINING_DATASET_COL" in window.AllKeysDict:
                ...
                if 'TABLE_TRAINING_DATASET' in window.AllKeysDict:
                    ...
                    window.add_row(window['TRAINING_DATASET_COL'], [[
                        sg.Frame('Training Dataset', key='FRAME_TRAINING', background_color='dark blue', pad=(0, 5),
                                 layout=[[sg.Table(values=df_training_dataset.values.tolist()[:30],
                                                   ...
                                                   justification="right",
                                                   num_rows=min(
                                                       df_training_dataset[df_training_dataset.columns[0]].count(), 20),
...
def show_patterns(window, tablein, feature_list):
    ...
    if "SHOW_PATTERNS_COL" in window.AllKeysDict:
        if 'TABLE_SHOW_PATTERNS' in window.AllKeysDict:
            ...
            window.add_row(window['SHOW_PATTERNS_COL'], [[
...
def setting_table_prediction(window):
    ...
    table_input_pattern_prediction.create_data(headers=len(df_training_dataset_header_list),
                                               cols=len(df_training_dataset_header_list), rows=2,
                                               size=2, inputdf=df_training_dataset.values.tolist()[9:11],
                                               headers_list=df_training_dataset_header_list)

    table_input_pattern_prediction.create_table(dimx=30, dimy=1, header_event=False,
                                                prediction_event=True)  # , background_color='yellow', background_color_list=background_color_list)
    newTable = sg.Column(table_input_pattern_prediction.table, background_color='black', pad=(0, 0), size=(1440, 40),
                         key='TABLE_2', scrollable=True)
...
list_column_bar = [
    ...
    #### TRAINING Options ####
    [sg.Text(f'TRAINING with {occurrences} OCCURRENCES, {utility} UTILITY, {max_card} MAX CARDINALITY',
             background_color="green", key="text_thresholds"), ],
    ...
    [sg.Radio('Feature\n (green)', "radio", default=True, key='-feature-'),  # GREEN
     ...
     sg.Button(enable_events=True, image_data=RUN_ICO, button_text="\n\n\n\n RUN Regression Model",
               key="-START_PREDICTION-", button_color=None, visible=False),
...
while True:
    ...
    if not isinstance(event, tuple) and event == 'About...':
        ...
        show_training_dataset(window,
                              "../Mortality_incidence_sociodemographic_and_clinical_data_in_COVID19_patients.xlsx")
        '''
        window.disappear()
        ...
        window['text_thresholds'].update(
            f'TRAINING with {occurrences} OCCURRENCES, {utility} UTILITY, {max_card} MAX CARDINALITY',
    ...
    if not isinstance(event, tuple) and event == '-RUN-':
        ...
        preprocess.preproc_col_into_word(list(df_training_dataset.values.tolist()), item_list)
        ...
        write_edb_fact(list(df_training_dataset.values.tolist()), df_training_dataset_header_list, clustering_list,
                       feature_list, item_list, target_list)
